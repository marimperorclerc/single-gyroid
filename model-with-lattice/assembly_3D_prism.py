...
parameters = [["sld", "1e-6/Ang^2", 126., [-inf, inf], "sld",
              ...
              ["length", "Ang", 5000, [0., inf], "volume",
               "Length"],
              ["a_lattice", "Ang", 100, [0., inf], "volume",
               "a_lattice"],
              ["n_a", "", 10, [1, 50], "volume",
               "assembly number along a_lattice"],
              ["b_lattice", "Ang", 100, [0., inf], "volume",
               "b_lattice"],
              ["n_b", "", 10, [1, 50], "volume",
               "assembly number along b_lattice"],
              ["c_lattice", "Ang", 100, [0., inf], "volume",
               "c_lattice"],
              ["n_c", "", 10, [1, 50], "volume",
...
def form_volume(n_sides, radius_average, length,a_lattice,b_lattice,c_lattice,n_a,n_b,n_c):
    """
...
def Iqabc(qa,qb,qc,n_sides,radius_average, length,a_lattice,b_lattice,c_lattice,n_a,n_b,n_c): # proportionnal to the volume**2
    """
    ...
    laue=ILaueqabc(qa,qb,qc,a_lattice,b_lattice,c_lattice,n_a,n_b,n_c)

    A = Fqabc(qa, qb, qc, n_sides, radius_average, length)

    intensity = coeff*laue*(np.abs(A))**2  # intensity is proportional to volume
    return intensity



def Iq(q, sld, sld_solvent, n_sides:int, radius_average:float, length:float, a_lattice:float, n_a:int, b_lattice:float, n_b:int, c_lattice:float, n_c:int, npoints_fibonacci:int= 1000):
    """
    ...
    # # Compute intensity
    intensity = Iqabc(qa.ravel(), qb.ravel(), qc.ravel(), n_sides, radius_average, length, a_lattice, n_a, b_lattice, n_b, c_lattice, n_c).reshape(qa.shape)
