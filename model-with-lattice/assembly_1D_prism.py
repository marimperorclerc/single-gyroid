...
parameters = [["sld", "1e-6/Ang^2", 126., [-inf, inf], "sld",
              ...
              ["length", "Ang", 5000, [0., inf], "volume",
               "Length"],
              ["a_lattice", "Ang", 100, [0., inf], "",
               "a_lattice"],
              ["n_size", "", 10, [1, 50], "",
               "Number in 1D assembly"]
...
def ILaueqabc(qa,qb,qc,a_lattice,n_size):
    a_vect=[a_lattice,0,0]
    return laue_cheb(qa*a_vect[0]+qb*a_vect[1]+qc*a_vect[2],n_size)**2

def Iqabc(qa,qb,qc,n_sides,radius_average, length,a_lattice,n_size): # proportionnal to the volume**2
    """
    ...
    intensity = n_size*ILaueqabc(qa,qb,qc,a_lattice,n_size)*(np.abs(A))**2  # intensity is proportional to volume
    return intensity



def Iq(q, sld, sld_solvent, n_sides:int, radius_average:float, length:float, a_lattice:float, n_size:int, npoints_fibonacci:int= 1000):
    """
    ...
    # # Compute intensity
    intensity = Iqabc(qa.ravel(), qb.ravel(), qc.ravel(), n_sides, radius_average, length, a_lattice, n_size).reshape(qa.shape)
